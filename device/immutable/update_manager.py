...
class UpdateManager:
    ...
    def check(self):
        ...
        if not update_cfg.get("enabled") or not manifest_url:
            return {"status": "disabled", "message": "update_disabled", "downloads": [], "reboot_required": False}
    ...
    def apply(self):
        ...
        for item in self.cached_plan["downloads"]:
            ...
            if not self.planner.verify_download(item["sha256"], payload):
                return {"status": "error", "message": "hash_mismatch", "error": item["path"], "reboot_required": False}
        ...
        self.config_store.save_update_state(state)
        return {"status": "ok", "message": "update_applied", "applied": applied, "reboot_required": state["reboot_required"]}
