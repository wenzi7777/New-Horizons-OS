...
def encode_value(value):
    ...
    if isinstance(value, (bytes, bytearray)):
        raw = bytes(value)
        return bytes([V_BYTES]) + struct.pack("<H", len(raw)) + raw
    if isinstance(value, (list, tuple)):
        parts = [bytes([V_LIST]), struct.pack("<H", len(value))]
        for item in value:
            parts.append(encode_value(item))
        return b"".join(parts)
    if isinstance(value, dict):
        items = list(value.items())
        parts = [bytes([V_DICT]), struct.pack("<H", len(items))]
    ...
    return bytes([V_STR]) + struct.pack("<H", len(raw)) + raw
...
def decode_value(data, offset=0):
    ...
    if value_type == V_LIST:
        _require(data, offset, 2)
        count = int(struct.unpack_from("<H", data, offset)[0])
        offset += 2
        result = []
