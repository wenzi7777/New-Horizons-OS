...
- `device/os/`：Normal OS source tree，會被 Recovery `write_os` 寫入裝置 `/os`。
...
- 使用 `type: "os"` 與 `target_root: "/os"`。
- 以 streaming SHA256 記錄每個檔案 hash。
- 排除 `.device/`、`device_state/`、`__pycache__/`、`.pyc` 與 `manifest.json`。

Recovery `write_os` 會先對 `/os/{path}` 算 hash；hash 相同就 skip，不同或不存在才下載到 `/ota_stage/os/...`，驗證成功後再套用到 `/os`。
...
```json
{
  ...
  "latest": "v0.1.19",
  "manifest_url": "https://raw.githubusercontent.com/wenzi7777/New-Horizons-OS/v0.1.19/device/os/manifest.json"
}
```
