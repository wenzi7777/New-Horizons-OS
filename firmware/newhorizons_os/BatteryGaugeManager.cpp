...
void BatteryGaugeManager::begin() { sample_ = BatteryGaugeSample(); detected_ = false; diagnostic_ = "not_polled"; lastPollMs_ = 0; profile_ = resolveBatteryProfile(false, false, BatteryIdClass::Unknown, BatteryProfileId::None); }
void BatteryGaugeManager::service(uint32_t nowMs) {
  if (lastPollMs_ && nowMs - lastPollMs_ < kPollMs) return;
  lastPollMs_ = nowMs;
#if !NHOS_BOARD_HAS_MAX17048
  sample_ = BatteryGaugeSample(); detected_ = false; diagnostic_ = "max17048_not_supported"; profile_ = resolveBatteryProfile(false, false, BatteryIdClass::Unknown, BatteryProfileId::None); return;
#else
  uint16_t vcell = 0, soc = 0, rate = 0;
  if (!readWord(0x02, vcell) || !readWord(0x04, soc) || !readWord(0x16, rate)) { sample_ = BatteryGaugeSample(); detected_ = false; diagnostic_ = "max17048_read_failed"; profile_ = resolveBatteryProfile(false, false, BatteryIdClass::Unknown, BatteryProfileId::None); return; }
  ...
  profile_ = resolveBatteryProfile(true, sample_.valid, readBatteryId(), BatteryProfileId::None);
#endif
}
