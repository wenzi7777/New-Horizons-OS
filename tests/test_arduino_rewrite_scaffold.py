...
class ArduinoRewriteScaffoldTests(unittest.TestCase):
    ...
    def test_missing_device_config_falls_back_to_board_default_matrix_layout(self):
        ...
        self.assertIn("data_.schemaVersion = 6", defaults_body.group("body"))
    ...
    def test_arduino_runtime_initializes_bmi270_and_streams_imu_payload(self):
        ...
        self.assertIn("imuDriver.begin()", imu_impl)
        self.assertIn("imuDriver.begin(BOSCH_ACCELEROMETER_ONLY)", imu_impl)
        self.assertIn("imuDriver.readMagneticField", magnetometer_impl)
        ...
        # FIFO mode was enabled but never drained; samples now come from one
        # bmi2_get_sensor_data() per service (see ImuDriver.h).
        self.assertIn("imuDriver.readAccelGyro(acc, gyr)", imu_impl)
    ...
    def test_external_ws2812b_controller_is_separate_from_internal_status_led_rmt(self):
        ...
        self.assertIn("dedicated RMT channel", external_header)
        ...
        self.assertIn("#if defined(NHOS_BOARD_V15F)", external)
        self.assertIn("Adafruit_NeoPixel", external_header)
        self.assertIn("adafruit_neopixel", external)
    ...
    def test_power_state_manager_scaffold_exists_and_tracks_soft_off_states(self):
        ...
        self.assertIn("ActionButtonGesture service", header)
    ...
    def test_v15f_board_led_settings_are_persisted_and_reported(self):
        ...
        self.assertIn("schemaVersion = 6", config)
